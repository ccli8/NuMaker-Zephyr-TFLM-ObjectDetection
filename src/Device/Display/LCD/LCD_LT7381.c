...
/* TODO: On zephyr, it seems EBI address region is regarded as normal memory.
 * The added memory barrier instruction works when DCACHE is not enabled.
 * Needs further check.
 */
#if defined(__ZEPHYR__)
#define LT7381_WRITE_REG(u32RegAddr) \
    do { \
        __DMB(); \
	(*((volatile uint16_t *)(CONFIG_LCD_EBI_ADDR+(LT7381_ADDR_CMD))) = (u32RegAddr)); \
	__DMB(); \
    } while (0)
#else
