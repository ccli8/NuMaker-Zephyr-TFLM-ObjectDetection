...
/* On zephyr, use k_sem */
#if defined(__ZEPHYR__)
static struct k_sem nu_pdma_sem_arr[NU_PDMA_MEMFUN_ACTOR_MAX];
#endif
...
static void nu_pdma_init(void)
{
    ...
    for (i = (PDMA_START + 1); i < PDMA_CNT; i++)
    {
        ...
        /* On zephyr, go in zephyr way */
#if defined(__ZEPHYR__)
        __ASSERT_NO_MSG(ARRAY_SIZE(nu_pdma_arr) == 2);
	IRQ_CONNECT(PDMA0_IRQn, 0, PDMA_IRQHandler, PDMA0, 0);
        irq_enable(PDMA0_IRQn);
	IRQ_CONNECT(PDMA1_IRQn, 0, PDMA_IRQHandler, PDMA1, 0);
        irq_enable(PDMA1_IRQn);
#else
...
static void nu_pdma_memfun_actor_init(void)
{
    ...
    for (i = 0; i < NU_PDMA_MEMFUN_ACTOR_MAX; i++)
    {
        ...
        if (-(1) != (nu_pdma_memfun_actor_arr[i].m_i32ChannID = nu_pdma_channel_allocate(PDMA_MEM)))
        {
            /* On zephyr, use k_sem */
#if defined(__ZEPHYR__)
            k_sem_init(&nu_pdma_sem_arr[i], 0, 1);
            nu_pdma_memfun_actor_arr[i].m_psSemMemFun = &nu_pdma_sem_arr[i];
#else
#if defined(OS_FREERTOS)
...
static void nu_pdma_memfun_cb(void *pvUserData, uint32_t u32Events)
{
    ...
    /* On zephyr, use k_sem */
#if defined(__ZEPHYR__)
    k_sem_give(psMemFunActor->m_psSemMemFun);
#else
#if defined(OS_FREERTOS)
...
/* On zephyr, use k_sem */
#if defined(__ZEPHYR__)
    k_sem_take(psMemFunActor->m_psSemMemFun, K_FOREVER);
#else
#if defined(OS_FREERTOS)
...
#endif      // defined(CONFIG_DISP_USE_PDMA)
